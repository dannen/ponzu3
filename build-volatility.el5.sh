...
if [ ! -f "/rpms/kernel-${KVER}.rpm" ]; then
  ...
  yum -y -q -e 0 install kernel-${KVER} kernel-devel-${KVER} kernel-headers-${KVER}
else
  echo "Local rpms found, installing..."
  yum install -y --nogpgcheck /rpms/kernel-${KVER}.rpm /rpms/kernel-devel-${KVER}.rpm /rpms/kernel-headers-${KVER}.rpm
